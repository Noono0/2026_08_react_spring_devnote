...
describe("ApiWorkspaceToolbar", () => {
  it("상단에서 Environment를 바꾸고 좌우 분할을 선택한다", () => {
    ...
    render(<ApiWorkspaceToolbar environments={[environment]} splitView="STACKED" onEnvironmentSelect={onEnvironmentSelect} onSplitViewChange={onSplitViewChange} onNewRequest={vi.fn()} onInterchangeOpen={vi.fn()} onCollectionRunnerOpen={vi.fn()} onHelpOpen={vi.fn()} />);
  ...
  it("새 요청과 테스트 시나리오 작업을 실행한다", () => {
    ...
    render(<ApiWorkspaceToolbar environments={[]} splitView="STACKED" onEnvironmentSelect={vi.fn()} onSplitViewChange={vi.fn()} onNewRequest={onNewRequest} onInterchangeOpen={vi.fn()} onCollectionRunnerOpen={onCollectionRunnerOpen} onHelpOpen={vi.fn()} />);
