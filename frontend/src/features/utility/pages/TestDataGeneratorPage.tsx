...
const initialRules: TestDataRule[] = [
  { ruleId: createUuid(), columnName: "id", type: "SEQUENCE", option: "", nullPercentage: 0, allowDuplicate: false },
  { ruleId: createUuid(), columnName: "name", type: "NAME", option: "", nullPercentage: 0, allowDuplicate: true },
  { ruleId: createUuid(), columnName: "email", type: "EMAIL", option: "", nullPercentage: 0, allowDuplicate: false },
  { ruleId: createUuid(), columnName: "role", type: "ENUM", option: "USER,ADMIN,SUPER_ADMIN", nullPercentage: 0, allowDuplicate: true },
];
...
export const TestDataGeneratorPage = () => {
  ...
  return (
    <section className="site-page utility-workbench-page">
      ...
      <section className="test-rule-panel"><header><div><h2>생성 규칙</h2><p>컬럼별 타입·옵션·Null·중복 여부를 설정합니다.</p></div><button type="button" className="ghost-button" onClick={() => setRules((current) => [...current, { ruleId: createUuid(), columnName: `field${current.length + 1}`, type: "STRING", option: "", nullPercentage: 0, allowDuplicate: true }])}>+ 규칙 추가</button></header>
