  ...
  return (
    <section className="api-workspace-page">
      <header className="api-workspace-titlebar">
        <div><span className="page-kicker">Developer Utility · API Client</span><div className="page-title-with-guide"><h1>API Workspace</h1><button type="button" className="learning-guide-icon-button" aria-label="API Workspace 도움말" onClick={() => setHelpOpen(true)}>?</button></div><p>요청을 작성하고 응답을 분석하며, 자주 쓰는 API를 회원별 작업 공간으로 정리합니다.</p></div>
      ...
      <ModalDialog isOpen={helpOpen} title="API Workspace 도움말" description="브라우저에서 안전하게 API 요청 흐름을 연습합니다." size="large" resizable resizeStorageKey="api-workspace-help" onRequestClose={() => setHelpOpen(false)}>
      ...
      <ModalDialog isOpen={codeDialogOpen} title="cURL 가져오기 · 코드 생성" description="현재 요청을 코드로 만들거나 cURL 명령을 현재 탭에 불러옵니다. Secret 원문은 생성 코드에 포함하지 않습니다." size="large" resizable resizeStorageKey="api-workspace-code" onRequestClose={() => setCodeDialogOpen(false)}>
